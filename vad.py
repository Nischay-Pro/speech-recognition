...
class VoiceActivityDetector():
    ...
    def __init__(self, wave_input_filename):
        ...
        self.sample_window = 0.5 #20 ms
        self.sample_overlap = 0.01 #10ms
        self.speech_window = 3 #half a second
    ...
    def detect_speech(self):
        ...
        while (sample_start < (len(data) - sample_window)):
            count = sample_start
            barlength = 60
            suffix = "Parsing Audio File"
            filledlen = int(round(barlength * count / float(total)))
            percents = round(100.0 * count / float(total))
            bar = '=' * filledlen + '-' * (barlength - filledlen)
